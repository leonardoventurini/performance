...
function id(value: unknown, path: string): string { const result = text(value, path, 128); if (!ID.test(result)) fail(path, 'has an invalid identifier'); return result; }
function oneOf<const Choice extends ScalarChoice>(value: unknown, path: string, choices: readonly Choice[]): Choice { if (!choices.includes(value as Choice)) fail(path, `must be one of: ${choices.join(', ')}`); return value as Choice; }
function integer(value: unknown, path: string, min: number, max: number): number { if (!Number.isSafeInteger(value) || (value as number) < min || (value as number) > max) fail(path, `must be an integer from ${min} through ${max}`); return value as number; }
function list<T>(value: unknown, path: string, validator: (entry: unknown, path: string) => T, max = 256, nonempty = false): T[] {
...
function unique<T>(values: T[], path: string): T[] { if (new Set(values).size !== values.length) fail(path, 'must contain unique entries'); return values; }
...
function normalized<T>(value: T): T { return frozen(structuredClone(value)); }

function boundedEjson(value: unknown, path: string, depth = 0): void {
...
export function validateValueRef(value: unknown, path = 'valueRef'): UnknownRecord {
...
function literalOrValueRef(value: unknown, path: string): void {
...
function selector(value: unknown, path: string): void {
  ...
  if (value.kind === 'field_equals') { record(value, path, ['kind', 'field', 'value']); const field = id(value.field, `${path}.field`); if (['runId', '_id'].includes(field)) fail(`${path}.field`, 'is interpreter-owned'); validateValueRef(value.value, `${path}.value`); return; }
  fail(`${path}.kind`, 'is not an allowlisted selector');
}
function mutation(value: unknown, path: string): void {
  if (value && typeof value === 'object' && 'kind' in value
      && typeof value.kind === 'string'
      && ['literal', 'parameter', 'coordinate', 'run', 'fixture', 'step'].includes(value.kind)) { validateValueRef(value, path); return; }
...
function transition(value: unknown, path: string): void {
  record(value, path, ['kind', 'path', 'value', 'amount', 'variant'], ['kind']);
  const kind = oneOf(value.kind, `${path}.kind`, ['insert', 'replace', 'delete', 'set_field', 'remove_field', 'increment_field', 'append_array', 'projection_variant']);
  if (['set_field', 'remove_field', 'increment_field', 'append_array'].includes(kind)) list(value.path, `${path}.path`, (part, p) => id(part, p), 16, true);
  if (['set_field', 'append_array'].includes(kind)) literalOrValueRef(value.value, `${path}.value`);
  if (kind === 'increment_field') literalOrValueRef(value.amount, `${path}.amount`);
  if (kind === 'projection_variant') id(value.variant, `${path}.variant`);
}
function refRecord(value: unknown, path: string): UnknownRecord {
...
function query(value: unknown, path: string): void {
...
function step(value: unknown, path: string): UnknownRecord {
...
function parameters(value: unknown, path: string): void {
  ...
  for (const [name, definition] of Object.entries(value)) {
    ...
    if (definition.type === 'integer') { record(definition, `${path}.${name}`, ['type', 'default', 'minimum', 'maximum']); const minimum = integer(definition.minimum, `${path}.${name}.minimum`, 0, GLOBAL_LIMITS.maximumPayloadBytes); const maximum = integer(definition.maximum, `${path}.${name}.maximum`, minimum, GLOBAL_LIMITS.maximumPayloadBytes); integer(definition.default, `${path}.${name}.default`, minimum, maximum); }
    else if (definition.type === 'enum') { record(definition, `${path}.${name}`, ['type', 'default', 'values']); const values = unique(list(definition.values, `${path}.${name}.values`, (v, p) => text(v, p, 128), 32, true), `${path}.${name}.values`); const defaultValue = text(definition.default, `${path}.${name}.default`, 128); if (!values.includes(defaultValue)) fail(`${path}.${name}.default`, 'must be an allowed value'); }
...
export function validateDeclarativeCaseDefinition(value: unknown, path = 'case'): UnknownRecord {
  ...
  const visitRefs = (node: unknown, nodePath: string, ownerIndex: number): void => {
    if (!node || typeof node !== 'object') return;
    if ('kind' in node && node.kind === 'step') {
      const reference = validateValueRef(node, nodePath);
      const stepId = id(reference.stepId, `${nodePath}.stepId`);
      const target = stepIndex.get(stepId);
      if (target === undefined) fail(`${nodePath}.stepId`, 'references an unknown step');
      if (target >= ownerIndex) fail(`${nodePath}.stepId`, 'must reference an earlier step');
    }
    for (const [key, child] of Object.entries(node)) visitRefs(child, `${nodePath}.${key}`, ownerIndex);
  };
  ...
  const oracles = list(value.oracles, `${path}.oracles`, (oracle, p) => {
    record(oracle, p, ['id', 'family', 'producer', 'expected', 'observed', 'failureReason', 'gate']);
    const oracleId = id(oracle.id, `${p}.id`);
    oneOf(oracle.family, `${p}.family`, DECLARATIVE_AUDIT_ORACLE_FAMILIES);
    const producer = oneOf(oracle.producer, `${p}.producer`, DECLARATIVE_AUDIT_PRODUCERS.filter((x) => x !== 'expected_model'));
    const expected = validateValueRef(oracle.expected, `${p}.expected`);
    const observed = refRecord(oracle.observed, `${p}.observed`);
    id(oracle.failureReason, `${p}.failureReason`);
    const gate = oneOf(oracle.gate, `${p}.gate`, ['hard', 'diagnostic']);
    return { ...oracle, id: oracleId, producer, expected, observed, gate };
  }, 64, true);
  unique(oracles.map((oracle) => oracle.id), `${path}.oracles`);
  if (!oracles.some((oracle) => oracle.gate === 'hard' && producers.includes(oracle.producer))) fail(`${path}.oracles`, 'must hard-gate a required independent producer');
  oracles.forEach((oracle, index) => {
    const expectedKind = oracle.expected.kind;
    const expectedStepId = expectedKind === 'step' ? id(oracle.expected.stepId, `${path}.oracles[${index}].expected.stepId`) : null;
    if (expectedStepId !== null && !stepIndex.has(expectedStepId)) fail(`${path}.oracles[${index}].expected.stepId`, 'references an unknown step');
    const observedStepId = id(oracle.observed.stepId, `${path}.oracles[${index}].observed.stepId`);
    if (observedStepId !== 'cleanup' && !stepIndex.has(observedStepId)) fail(`${path}.oracles[${index}].observed.stepId`, 'references an unknown step');
  });
  ...
  record(value.budget, `${path}.budget`, [...Object.keys(GLOBAL_LIMITS), 'maximumRetries']); for (const [key, maximum] of Object.entries(GLOBAL_LIMITS)) integer(value.budget[key], `${path}.budget.${key}`, 1, maximum); oneOf(value.budget.maximumRetries, `${path}.budget.maximumRetries`, [0, 1]); const maximumSteps = integer(value.budget.maximumSteps, `${path}.budget.maximumSteps`, 1, GLOBAL_LIMITS.maximumSteps); if (maximumSteps < steps.length) fail(`${path}.budget.maximumSteps`, 'must cover all declared steps'); oneOf(value.sharing, `${path}.sharing`, ['isolated']);
  return normalized(value);
}
...
export function validateDeclarativeAuditProfile(value: unknown, path = 'profile'): UnknownRecord {
...
export function validateAuditProfileCatalog(value: unknown, path = 'profiles'): UnknownRecord {
...
export function validateCapabilityCatalog(value: unknown, path = 'capabilities'): UnknownRecord {
  ...
  const capabilities = list(value.capabilities, `${path}.capabilities`, (entry, entryPath) => {
    ...
    id(entry.id, `${entryPath}.id`); const expectation = oneOf(entry.expectation, `${entryPath}.expectation`, ['supported', 'fallback_required', 'not_supported', 'out_of_scope']);
    const requiredCases = unique(list(entry.requiredCases, `${entryPath}.requiredCases`, (caseId, casePath) => id(caseId, casePath), 128), `${entryPath}.requiredCases`);
    const applicability = list(entry.applicability, `${entryPath}.applicability`, (scope, scopePath) => {
    ...
    if (['supported', 'fallback_required'].includes(expectation) && (requiredCases.length === 0 || applicability.length === 0)) fail(entryPath, 'must declare required cases and applicability'); return entry;
  }, 512, true);
...
export function validateNegativeControlDefinition(value: unknown, path = 'negativeControl'): UnknownRecord {
...
export function validateNegativeControlCatalog(value: unknown, path = 'negativeControls'): UnknownRecord {
